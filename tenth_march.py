...
class StrategistAgent:
    ...
    def parse(user_prompt):
        ...
        system_instruction = """
You are an adaptable, world-class Creative Director, Copywriter, and Visual Planner.
Analyze the user's prompt and return STRICTLY VALID JSON ONLY.
...
Output JSON format:
{
  ...
  "subheadline": "string (Write 2 concise marketing sentences that explain the product, event, or offer clearly. Max 10 words per sentence. Focus on the value being promoted in the user's prompt, not design descriptions. Avoid filler language.)",
...
2. Extract exact banner size if present in the prompt, such as 300x250, 1080x1350, 728x90. Otherwise use 1280x800.
3. Extract target audience exactly if given. Otherwise infer briefly.
4. Extract primary purpose exactly if given. Otherwise infer briefly.
5. Extract event date and event location exactly if provided, otherwise use empty strings.
6. Headline must be concise, uppercase, and directly relevant to the prompt.
7. Subheadline must be exactly 2 complete sentences. Focus on what the user is advertising, selling, or promoting.
8. Each sentence must clearly explain the value, benefit, or purpose described in the user's prompt. Do NOT describe the visual design, colors, or layout.
9. Do not use vague fluff such as "shine bright", "timeless elegance", "radiant beauty", "embrace the moment" unless explicitly requested.
10. Do not hallucinate discounts, product benefits, ingredients, or event details.
11. Always infer a hero object and a human subject from the prompt, even if the user does not explicitly mention them.
12. If no physical product is mentioned, infer a symbolic or thematic hero object that visually represents the topic.
13. subject_type must never be empty. Use values like "man", "woman", "young professional man", "young professional woman", "student", "speaker", etc.
14. hero_type must never be empty.
15. hero_prompt must be concrete, photorealistic, ad-friendly, and directly usable for image generation.
16. subject_prompt must be concrete, photorealistic, ad-friendly, and directly usable for image generation.
17. For AI, ethics, conference, education, awareness, festival, workshop, or community prompts, infer an appropriate hero and subject from context.
18. environment_prompt must describe a background scene only and must be empty of people.
19. The generated copy must reflect the event purpose, audience, and key attractions from the prompt.
20. product_type can be thematic if no literal product exists, such as "ai interface display", "conference stage element", "festival signage", "cat adoption poster", etc.

GOOD SUBHEADLINE EXAMPLES:
- "Explore AI ethics and future impact. Share your views on responsible innovation."
- "Find a pet friend at Reeves Hall. Join grooming and protection workshops."
- "Grab black friday sneaker deals now. Discover bold performance-inspired styles."

BAD SUBHEADLINE EXAMPLES:
- "Radiant hues celebrate festive elegance. Glamour glides across every smile."
- "Shine bright with timeless beauty. Embrace the future tonight."
"""
        ...
        try:
            ...
            return {
                ...
                "primary_purpose": "encourage engagement",
                "event_date": "",
                "event_location": "",
                "is_event": False,
                "event_name": "",
                "detected_color": "neutral",
                "environment_prompt": "clean premium studio background with cinematic lighting, empty of people",
                "product_type": "featured object",
                "marketing_copy": {
                    "headline": "SPECIAL OFFER",
                    "subheadline": "Discover the featured campaign today. Join now and explore more.",
                    "cta": "SHOP NOW"
                },
                "visual_plan": {
                    "hero_type": "featured object",
                    "hero_prompt": "professional advertising image of a featured object, premium commercial lighting, photorealistic, highly detailed, 8k",
                    "subject_type": "woman",
                    "subject_prompt": "professional advertising photo of a stylish woman in a premium campaign setting, photorealistic, realistic proportions, polished lighting, 8k",
                    "composition_intent": "hero bottom-left, subject right, text top-left"
                },
                "design_notes": {
                    "key_points": ["clear hierarchy", "premium look", "strong call to action"]
                }
            }
...
class LayoutAgent:
    ...
    def generate_layout(self, width, height, copy_data, layer_meta):
        ...
        3. "cta": "{cta_text}" (Must be near the bottom, but leave at least 40px of padding from the canvas edges. YOU MUST ALSO DEFINE ITS STYLE: 'bg_color', 'text_color', 'border_color' as RGBA arrays [R,G,B,A], and 'radius_style' as "pill", "rounded", or "square" based on the brand vibe. Example: A dark elegant brand might use [0,0,0,200] for bg).
        ...
        {{
            ...
            "cta": {{"x": 540, "y": 670, "w": 200, "h": 60, "bg_color": [0,0,0,180], "text_color": [255,255,255,255], "border_color": [255,255,255,200], "radius_style": "pill"}},
...
def removebg_cutout(logo_path: str, cache_dir: str = "./outputs/logo_cache") -> Image.Image:
    ...
    src = Path(logo_path)
    # FIX: Add the parent folder name to the cache file so "nike/logo" and "walmart/logo" don't collide
    cached = Path(cache_dir) / f"{src.parent.name}_{src.stem}_removebg.png"
    
    if cached.exists(): 
        print(f"   ♻️ Loading cached logo: {cached.name}")
...
def generate_hero_from_prompt(parsed, ref_images):
    clear_memory()
    hero_prompt = parsed.get("hero_prompt") or f"professional advertising image of {parsed.get('product_type', 'featured object')}, photorealistic, premium composition"
    negative = "human, hands, text, logo, watermark, blurry, ugly, deformed"
...
def generate_assets(parsed, ref_images, size):
    ...
    if parsed.get("product_image_path"):
        ...
        hero = generate_hero_from_prompt(parsed, ref_images)
    ...
    subject_type_lower = str(parsed["subject_type"]).lower()
    if "woman" in subject_type_lower:
        subject_neg = f"{base_neg}, (man:2.0)"
    elif "man" in subject_type_lower:
        subject_neg = f"{base_neg}, (woman:2.0)"
    else:
        subject_neg = base_neg
...
class LayoutManager:
    ...
    def composite(bg, subject, hero, parsed, layout_elements, logo_path=None):
        ...
        # --- C. CTA: AI-Driven Formatting + Centered Text ---
        ...
        if cta:
            ...
            # 3. Get AI coordinates and style configuration
            cta_box = layout_elements.get("cta", {})
            cta_x = cta_box.get('x', (w // 2) - (pill_w // 2))
            cta_y = min(cta_box.get('y', h - 130), h - pill_h - 30) 
            
            # Helper to safely parse RGBA from JSON to tuple
            def parse_rgba(val, default):
                try:
            ...
            # Read formatting from the Agent
            bg_color = parse_rgba(cta_box.get('bg_color'), (255, 255, 255, 60))
            text_color = parse_rgba(cta_box.get('text_color'), (255, 255, 255, 255))
            border_color = parse_rgba(cta_box.get('border_color'), (255, 255, 255, 150))
            radius_style = cta_box.get('radius_style', 'pill')
            
            if radius_style == "square":
                corner_radius = 0
            elif radius_style == "rounded":
                corner_radius = 15
            else: # "pill" or fallback
                corner_radius = pill_h // 2
            
            # 4. Draw the AI-Formatted Button
            ...
            draw.text((text_x, text_y), cta, font=c_font, fill=text_color)
...
def main():
    ...
    visual_plan = parsed_data.get("visual_plan", {})

    inferred_subject_type = (
        visual_plan.get("subject_type")
        or parsed_data.get("subject_type")
        or "woman"
    )

    inferred_subject_prompt = (
        visual_plan.get("subject_prompt")
        or (
            f"professional advertising photo of a {inferred_subject_type}, "
            f"in a scene matching {parsed_data.get('environment_prompt', 'a premium studio setting')}. "
            f"They are posed naturally for a high-end marketing banner. "
            f"Photorealistic, polished lighting, premium composition, realistic proportions, 8k."
        )
    )

    inferred_hero_prompt = (
        visual_plan.get("hero_prompt")
        or (
            f"professional advertising image of {visual_plan.get('hero_type') or parsed_data.get('product_type', 'featured object')}, "
            f"photorealistic, premium composition, commercial lighting, highly detailed, 8k"
        )
    )
    ...
    parsed = {
        ...
        "subject_prompt": inferred_subject_prompt,
        "hero_prompt": inferred_hero_prompt,
        "hero_type": visual_plan.get("hero_type") or parsed_data.get("product_type", "featured object"),
        "composition_intent": visual_plan.get("composition_intent", ""),
        ...
        "primary_purpose": parsed_data.get("primary_purpose", ""),
        "event_date": parsed_data.get("event_date", ""),
        "event_location": parsed_data.get("event_location", "")
    }
    ...
    banner_info = parsed_data.get("banner", {})
    CANVAS_W = int(banner_info.get("width", 1280))
    CANVAS_H = int(banner_info.get("height", 800))
