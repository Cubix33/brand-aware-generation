...
# Use-case comes from your AI analysis inside generate_hero_v9_for_v39.
# We need it earlier, so do a light detection here:
    use_case, festival_name, ai_products = detect_use_case_with_ai(parsed.get("original_prompt", ""), parsed)
    ...
    def composite(bg, subject, hero, parsed, layout_elements, logo_path=None):
        ...
        if hero:
            ...
            try:
                ...
                hero_rgba = fit_image_into_square_with_padding(hero_rgba, p_size, pad_ratio=0.10)
                hero = hero_rgba
            except Exception:
...
def save_layout_debug_image(bg_image, layout_elements, output_path):
    """
    Draws bounding boxes from PosterLlama on top of the generated background
    to visualize where the AI wanted to place elements.
    """
    if not layout_elements:
        print("⚠️ No layout elements to visualize.")
        return None

    # Create a copy to draw on so we don't mess up the original BG
    debug_img = bg_image.copy().convert("RGBA")
    ...
    print(f"   📐 Drawing {len(layout_elements)} debug boxes...")

    for elem in layout_elements:
        etype = elem.get("type", "unknown")
        bbox = elem.get("bbox", [0,0,0,0])
        
        # PosterLlama outputs normalized x,y,w,h (0.0 to 1.0)
        # x,y is usually Top-Left corner
        bx, by, bw, bh = bbox
        ...
        col = colors.get(etype, "cyan")

        # Draw the rectangle outline
        draw.rectangle([x1, y1, x2, y2], outline=col, width=5)

        # Draw a background box for the label text for readability
        label = f"{etype}"
        text_bbox = draw.textbbox((x1, y1), label, font=font)
        # offset text box slightly up so it sits on the line
        text_bg = [x1, y1 - 25, x1 + (text_bbox[2]-text_bbox[0]) + 10, y1]
        
        draw.rectangle(text_bg, fill=col)
...
def main():
    ...
    # ============================================================================
    # 💾 PROCESS AND SAVE SEPARATE LAYERS
    # ============================================================================
    ...
    if subject:
        ...
        layer_meta["subject"] = {"x": CANVAS_W - s_w + 50, "y": CANVAS_H - s_h, "width": s_w, "height": s_h}
    ...
    # ============================================================================
    # 🖼️ COMPOSITE & VISUALIZE LAYOUT
    # ============================================================================
    
    # Generate Layout Data
    layout_data = layout_gen.generate_layout()
    
    # --- E. NEW: Save Layout Visualization ---
    ...
    json_response = {
        ...
        "assets": {
            ...
            "layers": layer_paths,
            "debug_layout": debug_path  # <--- Added path to new image here
        },
